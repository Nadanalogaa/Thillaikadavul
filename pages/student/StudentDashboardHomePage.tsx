                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {(family[activeIdx]?.courses || []).map((courseName, i) => {
                            ...
                            return (
                                <div key={courseName} className={`relative rounded-2xl p-4 border-2 ${colors.border} ${colors.bg} shadow-lg hover:shadow-xl transition-all duration-300`}>
                                    <div className={`text-lg font-semibold ${colors.text}`}>{courseName}</div>
                                    <div className="mt-2 text-sm font-semibold text-gray-700">{allocatedByCourse.length > 0 ? 'Allocated Times' : 'Preferred Times'}</div>
                                    <div className="mt-1 space-y-1 text-sm text-gray-600">
                                        {times.length > 0 ? times.slice(0, 3).map((t, idx2) => <div key={idx2}>{t}</div>) : <div className="text-gray-400">No times yet</div>}
                                    </div>
                                    {courseImage ? (
                                        <img src={courseImage} alt={courseName} className="absolute right-3 bottom-2 w-20 h-20 object-contain pointer-events-none select-none rounded-lg shadow-sm" />
                                    ) : (
                                        <div className="absolute right-3 bottom-2 w-20 h-20 flex items-center justify-center bg-white/70 backdrop-blur-sm rounded-lg shadow-sm border border-gray-200">
